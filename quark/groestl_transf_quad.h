...
void to_bitslice_quad(uint32_t *const __restrict__ input, uint32_t *const __restrict__ output)
{
	...
	#pragma unroll
	for (int i = 0; i < 8; i++) {
		input[i] = __shfl_sync(0xFFFFFFFFu, (int)input[i], n ^ (3*(n >=1 && n <=2)), 4);
		other[i] = __shfl_sync(0xFFFFFFFFu, (int)input[i], (threadIdx.x + 1) & 3, 4);
		input[i] = __shfl_sync(0xFFFFFFFFu, (int)input[i], threadIdx.x & 2, 4);
		other[i] = __shfl_sync(0xFFFFFFFFu, (int)other[i], threadIdx.x & 2, 4);
		if (threadIdx.x & 1) {
...
void from_bitslice_quad(const uint32_t *const __restrict__ input, uint32_t *const __restrict__ output)
{
	...
	for (int i = 0; i < 16; i+=2) {
		...
		output[i] = __byte_perm(output[i], __shfl_sync(0xFFFFFFFFu, (int)output[i], (threadIdx.x+1)&3, 4), 0x7610);
		output[i+1] = __shfl_sync(0xFFFFFFFFu, (int)output[i], (threadIdx.x+2)&3, 4);
